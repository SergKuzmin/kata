"""
For this kata you will be using some meta-programming magic to create a new Thing object. This object will allow you to
define things in a descriptive sentence like format.
...
>> Note: Python, unlike Ruby and Javascript, doesn't have a pretty syntax for blocks of expressions and a forEach method
for iterables. So you should implement this behaviour yourself.
...
class Thing (object):
    ...
    def __getattribute__(self, item):
        ...
        if self.change:
            if self.state:
                super().__setattr__(f'is_a_{item}', True)
            else:
                super().__setattr__(f'is_a_{item}', False)
            self.change = False
